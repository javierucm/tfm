...
data <- read.table(file.path(current_dir, "TeTranscript/10month/TFM_JLR_Long10MonthYoungvsOldTreated_gene_TE_analysis.txt"), header=T, row.names=1)
...
data$category[data$padj < 0.05 & data$log2FoldChange >= 1] <- "Upregulados"
data$category[data$padj < 0.05 & data$log2FoldChange <= -1] <- "Downregulados"
data$category[data$padj < 0.05 & abs(data$log2FoldChange) < 1] <- "Cambios no marcados"

# Crear el volcano plot ajustado
png(file.path(current_dir,"Figuras/Fig 5:Volcanoplot_All_genes_Paper_YoungVsOldTreated_10month.png"), width = 800, height = 600)
ggplot(data, aes(x = log2FoldChange, y = -log10(padj), color = category)) +
  ...
  labs(title = "Fig 5: Genes: 22 months +doxycycline(10month) / 3 months -doxycycline",
       x = "Log2 Fold Change",
       y = "-log10(p-value adjusted)") +
  xlim(c(-10, 10)) +
  ylim(c(0, 7)) +
  annotate("text", x = 5, y = 6.5, label = paste(nrow(data[data$category == "Upregulados", ]), "Upregulados"), 
           color = "red", size = 6, fontface = "bold") + # Ajusta tamaño y estilo
  annotate("text", x = -5, y = 6.5, label = paste(nrow(data[data$category == "Downregulados", ]), "Downregulados"), 
