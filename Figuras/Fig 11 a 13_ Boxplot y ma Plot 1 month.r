...
# Cargar los datos
data_no_treated <- read.table(file.path(current_dir, "TeTranscript/1month/TFM_JLR_YoungvsOld_NoTreated_gene_TE_analysis.txt"), header = TRUE, sep = "\t", row.names = 1)
data_treated <- read.table(file.path(current_dir, "TeTranscript/1month/TFM_JLR_YoungvsOldTreated_gene_TE_analysis.txt"), header = TRUE, sep = "\t", row.names = 1)
...
# Crear el boxplot
png(file.path(current_dir, "Figuras/Fig 11: Boxplot 1 month.png"), width = 800, height = 600)
ggplot(long_data, aes(x = Condition, y = value, fill = Condition)) +
  geom_boxplot() +
  labs(
    title = "Fig 11: Distribución de log2FoldChange por cond (10 month treatment)",
    x = "Condición",
    y = "log2FoldChange",
    fill = "Condición"
  ) +
  ...
  scale_fill_manual(
    values = c("blue", "red"),
    labels = c("26 months -doxycycline / 3 months -doxycycline", 
               "26 months +doxycycline / 3 months -doxycycline")
  ) +
  theme(
...
png(file.path(current_dir,"Figuras/Fig 12: MA Plot no treated Condition (26 month old).png"), width = 800, height = 600)
...
plot(TE_no_treated$baseMean,
     ...
     main = "Fig 12: MA Plot - No Treatment Condition (26 month old)",
...
png(file.path(current_dir,"Figuras/Fig 13: MA Plot treated Condition (1 month treatment - 26 month old).png"), width = 800, height = 600)
plot(TE_treated$baseMean,
     ...
     xlab = "Mean of normalized counts (log scale)",
     ylab = "log2 Fold Change",
     main = "Fig 13: MA Plot - Treatment Condition (1 month treatment - 26 month old)",
     cex.lab = 1.3,    # Tamaño de las etiquetas de los ejes
     cex.main = 1.5,     # Tamaño del título
     cex.axis = 1.3    # Tamaño de los números en los ejes
)
