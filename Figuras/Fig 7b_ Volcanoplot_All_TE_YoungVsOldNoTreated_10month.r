...
data <- read.table(file.path(current_dir, "TeTranscript/10month/TFM_JLR_Long10MonthYoungvsOldNoTreated_gene_TE_analysis.txt"), header=T, row.names=1)
...
png(file.path(current_dir,"Figuras/Fig 7b: Volcanoplot_All_TE_Paper_YoungVsOldNoTreated_10month.png"), width = 800, height = 600)
ggplot(data, aes(x = log2FoldChange, y = -log10(padj), color = category)) +
  ...
  geom_vline(xintercept = c(-0.585, 0.585), linetype = "solid", color = "black") +
  ...
  labs(title = "Fig 7b: TE: 22 months -doxycycline(10month) / 3 months -doxycycline",
       x = "Log2 Fold Change",
  ...
  theme(
    ...
    legend.position = "none"                             # Mantener sin leyenda
  )
