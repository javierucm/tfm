...
data1 <- read.table(file.path(current_dir, "TeTranscript/10month/TFM_JLR_Long10MonthYoungvsOldNoTreated.cntTable"), header=T, row.names=1)
data2 <- read.table(file.path(current_dir, "TeTranscript/10month/TFM_JLR_Long10MonthYoungvsOldTreated.cntTable"), header=T, row.names=1)
...
library(ggplot2)
png(file.path(current_dir, "Figuras/Fig 6b:PCA_All_TE_10month.png"), width = 800, height = 600)
ggplot(pca_coordinates, aes(x = PC1, y = PC2, color = Group)) +
  geom_point(size = 4, alpha = 0.8) +
  labs(title = "Fig 6b: PCA of TE Normalized Counts (10 month treatment)",
  ...
  scale_color_manual(values = c("Control" = "orange", "Old_NoTreated" = "red", "Old_Treated" = "blue"),
                     labels = c("3 months -doxycycline", "22 months -doxycycline", "22 months +doxycycline (10 month)")
  ) +
  theme(
