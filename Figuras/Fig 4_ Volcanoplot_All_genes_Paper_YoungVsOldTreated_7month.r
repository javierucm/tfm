...
data <- read.table(file.path(current_dir, "TeTranscript/7month/TFM_JLR_Long7MonthYoungvsOldTreated_gene_TE_analysis.txt"), header=T, row.names=1)
...
png(file.path(current_dir,"Figuras/Fig 4: Volcanoplot_All_genes_Paper_YoungVsOldTreated_7month.png"), width = 800, height = 600)
ggplot(data, aes(x = log2FoldChange, y = -log10(padj), color = category)) +
  ...
  labs(title = "Fig 4: Genes: 22 months +doxycycline(7month) / 3 months -doxycycline",
       x = "Log2 Fold Change",
