...
data <- read.table(file.path(current_dir, "TeTranscript/1month/TFM_JLR_YoungvsOld_NoTreated_gene_TE_analysis.txt"), header=T, row.names=1)
...
png(file.path(current_dir,"Figuras/Fig 7: Volcanoplot_All_TE_Paper_YoungVsOldNoTreated_1month.png"), width = 800, height = 600)
ggplot(data, aes(x = log2FoldChange, y = -log10(padj), color = category)) +
  ...
  labs(title = "Fig 7: TE: 26 months -doxycycline(1month) / 3 months -doxycycline",
       x = "Log2 Fold Change",
